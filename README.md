...
| `GET`  | `/tickets` | Admin-dashboard list. Filters: `status` (single or `status[]=` multi), `assigned=me\|unassigned`, `admin_id`, `created_by`, `platform`, `is_valid`, `category_id` (+ `include_subcategories=1` for descendants), `sub_category_id`, `sub_sub_category_id`, `type_id`, `season_id`, `zone_id`, `has_replies`, `has_notes`, `has_attachments`. Search: `q` (subject/description/ticket_number/first_name/last_name/email/phone). Date ranges: `created_from`/`created_to`, `occurrence_from`/`occurrence_to`, `closed_from`/`closed_to`. Sort: `sort=latest\|oldest\|updated\|closed`. Soft-delete scope: `with_trashed=1` or `only_trashed=1`. Invalid `status` values are silently dropped. |
